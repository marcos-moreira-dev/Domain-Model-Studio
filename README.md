...
> Estado documental vivo post Tanda 27: para continuar el refactor leer `docs/README.md`, `docs/documentacion/MAPA_DOCUMENTACION_VIVA.md`, `docs/desarrollo/PLAN_TANDAS_ACTUAL.md`, `docs/diagnostico/ESTADO_AUDITORIA_ACTUAL.md` y `scripts/README.md`.
> El repositorio conserva Markdown que explica capacidades vigentes, contratos, pruebas o fronteras actuales; la bitácora histórica de tandas pasadas se elimina salvo valor concreto de auditoría o compatibilidad.
...
También puede exportarse como PDF o Markdown para entregar una versión formal del levantamiento de información.
...
Los diagramas visuales permiten mover elementos, revisar estructura, seleccionar, aplicar estilos por categoría y exportar resultados. El enfoque es guiado por el tipo de artefacto: un mapa de módulos no se edita igual que un caso de uso, un despliegue técnico o un wireframe administrativo.
...
El proyecto está en etapa de **MVP funcional avanzado / pre-RC local**. Ya permite abrir ejemplos, editar varios tipos de artefacto, consultar guía académica, exportar documentación y trabajar con un conjunto amplio de diagramas. El módulo de UML Clases cuenta con estabilización para importación desde código, vistas internas, render seguro, exportación protegida y navegación hacia archivos fuente. UML Secuencia ya incorpora fragmentos combinados (`alt`, `opt`, `loop`, `par`, `break`, `critical`, `ref`) y la familia UENS fue saneada como demo oficial. Antes de tratarlo como release candidate, ejecutar la validación local Windows indicada en `scripts\16-release-candidate.bat` y completar los reportes de `docs/testeo/reportes/`.
...
## Estado de repositorio limpio

README raíz vigente para GitHub con rutas relativas de assets, LICENSE conservador y pom.xml alineado al alcance multiproyecto.
