    ...
    private void installKeyboardShortcuts() {
        ...
        root.addEventFilter(KeyEvent.KEY_PRESSED, event -> {
            if ((event.getCode() == KeyCode.DELETE || event.getCode() == KeyCode.BACK_SPACE)
                    && adapter instanceof CanvasVisualCommentPort visualCommentPort
                    && visualCommentPort.deleteSelectedVisualComment()) {
                refreshPreservingViewport(); event.consume(); return;
            }
            if ((event.getCode() == KeyCode.DELETE || event.getCode() == KeyCode.BACK_SPACE)
                    && interactionProfile.supportsBendPoints()
                    && adapter.supportsBendPoints()
                    && adapter.selection().selectedBendPoint().isPresent()) {
                bendPointController.removeSelected(); refreshPreservingViewport(); event.consume();
            }
        });
    }
