...
public final class CanvasNodeInteractionCoordinator {
        ...
        rendered.addEventFilter(MouseEvent.MOUSE_CLICKED, event -> {
            if (event.getButton() == MouseButton.PRIMARY) {
                if (event.getClickCount() >= 2
                        && adapter instanceof CanvasNodeDoubleClickPort doubleClickPort
                        && doubleClickPort.handleNodeDoubleClick(node.id(), interactionRole(event, rendered))) {
    ...
    private String interactionRole(MouseEvent event, Node renderedRoot) {
        Object target = event.getTarget();
        if (!(target instanceof Node node)) {
            return "";
        }
        Node current = node;
        while (current != null && current != renderedRoot) {
            Object role = current.getProperties().get("canvasInteractionRole");
