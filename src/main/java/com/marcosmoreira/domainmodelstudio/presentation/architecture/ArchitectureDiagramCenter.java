...
import com.marcosmoreira.domainmodelstudio.presentation.interactivecanvas.VisualCommentCanvasAdapter;
import com.marcosmoreira.domainmodelstudio.presentation.interactivecanvas.VisualCommentRenderKit;
...
/** Centro visual de C4/despliegue montado sobre la superficie canónica. */
final class ArchitectureDiagramCenter {

    private final ArchitectureDiagramViewModel viewModel;
    private final VisualCommentCanvasAdapter canvasAdapter;
    private final VisualCommentRenderKit renderKit;
    ...
    ArchitectureDiagramCenter(ArchitectureDiagramViewModel viewModel) {
        this.viewModel = Objects.requireNonNull(viewModel, "viewModel");
        this.canvasAdapter = new VisualCommentCanvasAdapter(new ArchitectureCanvasAdapter(viewModel));
        this.renderKit = new VisualCommentRenderKit(new ArchitectureRenderKit());
        this.surfaceView = new InteractiveCanvasSurfaceView(canvasAdapter, renderKit, surface);
        this.pngExporter = new InteractiveCanvasPngExporter(canvasAdapter, renderKit, "architecture-export-surface", this::exportHeaderMetadata);
        build();
        bindViewModel();
    ...
    void activateVisualCommentTool() {
        canvasAdapter.activateCommentPlacement();
    }

    boolean reorderSelectedVisualComment(VisualLayerOrderCommand command) {
        return canvasAdapter.reorderSelectedVisualComment(command);
    }

    boolean resizeSelectedVisualComment(VisualNodeSizeCommand command) {
        return canvasAdapter.resizeSelectedVisualComment(command.factor());
    }
    ...
    private void build() {
        root.getStyleClass().add("architecture-diagram-center");
        root.setCenter(surfaceView.root());
    }

    private void bindViewModel() {
        viewModel.nodes().addListener((ListChangeListener<ArchitectureNode>) change -> refreshCanvasOnly());
        viewModel.edges().addListener((ListChangeListener<ArchitectureEdge>) change -> refreshCanvasOnly());
    }
