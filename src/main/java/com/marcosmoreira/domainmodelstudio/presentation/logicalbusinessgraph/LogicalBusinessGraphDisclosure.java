...
/** Seccion colapsable efimera para listas densas del SideDock del grafo logico. */
final class LogicalBusinessGraphDisclosure {

    private LogicalBusinessGraphDisclosure() {
    }
    ...
    static TitledPane section(
        ...
        pane.getStyleClass().add("logical-business-graph-disclosure");
    ...
    static VBox body() {
        ...
        body.getStyleClass().add("logical-business-graph-disclosure-body");
        return body;
    }

    private static VBox header(String title, String countText, String metaText) {
        VBox header = new VBox(1);
        header.getStyleClass().add("logical-business-graph-disclosure-header");
        Label titleLabel = new Label(headerTitle(title, countText));
        titleLabel.setWrapText(true);
        titleLabel.getStyleClass().add("logical-business-graph-disclosure-title");
        header.getChildren().add(titleLabel);
        if (metaText != null && !metaText.isBlank()) {
            Label meta = new Label(clean(metaText));
            meta.setWrapText(true);
            meta.getStyleClass().add("logical-business-graph-disclosure-meta");
