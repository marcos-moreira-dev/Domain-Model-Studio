...
final class WireframeDiagramCenter {
    ...
    private final WireframeCanvasAdapter wireframeAdapter;
    ...
    WireframeDiagramCenter(WireframeViewModel viewModel) {
        ...
        this.wireframeAdapter = new WireframeCanvasAdapter(viewModel);
        this.canvasAdapter = new VisualCommentCanvasAdapter(wireframeAdapter);
        this.renderKit = new VisualCommentRenderKit(new WireframeRenderKit());
    ...
    private void installWireframeShortcuts() {
        root.addEventFilter(KeyEvent.KEY_PRESSED, event -> {
            if (event.getCode() == KeyCode.DELETE || event.getCode() == KeyCode.BACK_SPACE) {
                if (canvasAdapter.deleteSelectedVisualComment()) {
                    refreshCanvasOnly();
                    event.consume();
                    return;
                }
                wireframeAdapter.deleteSelectedItems();
                refreshCanvasOnly();
                event.consume();
                return;
            }
