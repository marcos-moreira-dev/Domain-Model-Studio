...
import static com.marcosmoreira.domainmodelstudio.presentation.toolbar.DiagramToolbarActionFactory.visualCommentAction;
 ...
 *
 * <p>R1: la lista concreta de acciones ya no vive aquÃ­; se delega a contributors por familia para evitar
...
public final class DefaultDiagramToolbarActionProvider implements DiagramToolbarActionProvider {
    ...
    private List<DiagramToolbarAction> supportedActions(DiagramTypeId diagramTypeId, List<DiagramToolbarAction> actions) {
        ...
        if (visualCommentAvailable(diagramTypeId, profile)) {
            DiagramToolbarAction action = visualCommentAction();
    ...
    private boolean visualCommentAvailable(DiagramTypeId diagramTypeId, DiagramInteractionProfile profile) {
        return !DiagramTypeId.CONCEPTUAL_MODEL.equals(diagramTypeId)
                && profile.supportsNodeDragging()
                && !profile.supportsDocumentEditing()
                && !profile.supportsMatrixEditing();
