...
public final class MainShellCommandHandler {
    ...
    public void requestExportPdf() { exportCommandHandler.requestExportPdf(); }
    public void requestExportDataDictionaryPdf() { requestExportPdf(); }
    ...
    public void requestActivateVisualCommentTool() {
        Optional<DiagramProject> activeProject = activeProjectForOutput();
        if (activeProject.isPresent() && specializedWorkspaces.activateVisualCommentTool(activeProject.get())) {
            return;
        }
        shellState.updateStatus("Comentario visual disponible solo en diagramas visuales editables.");
    }
