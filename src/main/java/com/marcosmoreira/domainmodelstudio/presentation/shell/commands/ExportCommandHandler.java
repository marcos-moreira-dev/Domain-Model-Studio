...
public final class ExportCommandHandler {
    ...
    public void requestExportPdf() {
        ...
        try {
            java.nio.file.Path exported = context.applicationServices().exportServices().exportDataDictionaryPdfUseCase().export(
                    output.dataDictionaryDocument()
                            .orElseThrow(() -> new IllegalStateException("La salida activa no tiene PDF exportable.")),
                    file.toPath());
