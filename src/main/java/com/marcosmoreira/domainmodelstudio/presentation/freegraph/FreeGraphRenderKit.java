...
/** Renderizador de nodos rectangulares y relaciones del Grafo libre. */
public final class FreeGraphRenderKit implements InteractiveCanvasRenderKit {
    ...
    public Node renderNode(
        ...
        group.getStyleClass().add("free-graph-node-visual-group");

        Rectangle body = new Rectangle(0.0, 0.0, bounds.width(), bounds.height());
        body.setArcWidth(0.0);
        body.setArcHeight(0.0);
        body.getStyleClass().addAll("free-graph-node-body", "diagram-node", "diagram-node-free-graph");
        if (selected) {
            body.getStyleClass().addAll("diagram-node-selected", "free-graph-node-selected");
        }

        Label title = new Label(clamp(node.title(), 56));
        title.getStyleClass().add("free-graph-node-title");
        title.setWrapText(true);
        title.setMaxWidth(Math.max(72.0, bounds.width() - 28.0));
        title.setPrefWidth(Math.max(72.0, bounds.width() - 28.0));
        title.relocate(14.0, 12.0);

        Label subtitle = new Label(clamp(firstLine(node.subtitle()), 110));
        subtitle.getStyleClass().add("free-graph-node-subtitle");
        subtitle.setWrapText(true);
        subtitle.setMaxWidth(Math.max(72.0, bounds.width() - 28.0));
        subtitle.setPrefWidth(Math.max(72.0, bounds.width() - 28.0));
        subtitle.setPrefHeight(Math.max(38.0, bounds.height() - 50.0));
        subtitle.relocate(14.0, 40.0);
        subtitle.setVisible(!subtitle.getText().isBlank());
        subtitle.setManaged(!subtitle.getText().isBlank());

        if (selected) {
            title.getStyleClass().add("free-graph-node-text-selected");
            subtitle.getStyleClass().add("free-graph-node-text-selected");
        }
        group.getChildren().addAll(body, title, subtitle);
        if (selected) {
    ...
    public Node renderConnector(
        ...
        group.getStyleClass().add("free-graph-connector-visual-group");
        group.setUserData(connector.id());
        return group;
    }

    private Group renderNormalConnector(
            InteractiveCanvasConnector connector,
            InteractiveCanvasAdapter adapter,
            NodeLayout source,
            NodeLayout target,
            DiagramConnectorStyle style,
            DiagramDrawingFacade drawingFacade
    ) {
    ...
    private Group renderSelfLoop(NodeLayout layout, DiagramConnectorStyle style) {
        ...
        Path path = new Path(
                ...
                new CubicCurveTo(
                        loop.control1().getX(), loop.control1().getY(),
                        loop.control2().getX(), loop.control2().getY(),
                        loop.end().getX(), loop.end().getY())
        );
        path.getStyleClass().addAll(style.lineStyleClasses());
        path.getStyleClass().add("free-graph-connector-line");

        Node arrow = createSelfLoopArrow(loop, style);
        return new Group(path, arrow);
    }

    private Node createSelfLoopArrow(SelfLoop loop, DiagramConnectorStyle style) {
        if (style.arrowKind() == DiagramArrowKind.NONE) {
            return new Group();
        }
        ...
        if (style.arrowKind() == DiagramArrowKind.OPEN || style.arrowKind() == DiagramArrowKind.HOLLOW_TRIANGLE) {
            arrow.getStyleClass().add("diagram-connector-arrow-hollow");
        }
        return arrow;
    }

    private DiagramConnectorStyle connectorStyle(InteractiveCanvasConnector connector, boolean selected) {
        return DiagramConnectorStyle
                .directed("free-graph-connector-" + connector.kind(), selected)
                .withArrowKind(connector.kind().contains("undirected") ? DiagramArrowKind.NONE : DiagramArrowKind.FILLED_TRIANGLE)
                .withLineStyleClass("free-graph-connector-line");
    }
    ...
    private static String firstLine(String value) {
        ...
        return index < 0 ? value.strip() : value.substring(0, index).strip();
    }
