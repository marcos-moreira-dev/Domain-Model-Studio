...
public final class DataDictionaryEditorView {
    ...
    private final ListView<DataDictionaryEntity> entityList = new ListView<>();
    private final Label entityIndexSummary = new Label();
    ...
    private Parent buildEntityPanel() {
        VBox panel = section("Índice documental");
        Label note = helper("Selecciona una entidad para revisar sus campos, metadatos y reglas de datos.");
        entityIndexSummary.getStyleClass().add("data-dictionary-index-summary");
        entityIndexSummary.setWrapText(true);
        entityList.setItems(viewModel.entities());
        entityList.getStyleClass().add("data-dictionary-entity-list");
        SideDockCollectionSizingPolicy.configureListViewForExternalSideDockScroll(entityList);
        entityList.setCellFactory(ignored -> new ListCell<>() {
            @Override
            protected void updateItem(DataDictionaryEntity item, boolean empty) {
                super.updateItem(item, empty);
                setText(null);
                setGraphic(empty || item == null ? null : entityListGraphic(item));
            }
        });
        panel.getChildren().addAll(note, entityIndexSummary, entityList);
        refreshEntityIndexSummary();
        return panel;
    }
    ...
    private Parent buildDocumentPanel() {
        ...
        tabs.getStyleClass().add("data-dictionary-output-tabs");
        Tab documentTab = new Tab("Documento", documentScroll);
        documentTab.setClosable(false);
        Tab tableTab = new Tab("Campos de entidad", buildFieldTablePanel());
        tableTab.setClosable(false);
        tabs.getTabs().addAll(documentTab, tableTab);
        VBox.setVgrow(tabs, Priority.ALWAYS);

        panel.getChildren().addAll(hint, badges, tabs);
        return panel;
    }
    ...
    private Parent buildFieldTablePanel() {
        ...
        panel.getChildren().addAll(title, note, fieldTable);
        return panel;
    }
    ...
    private Parent buildPropertiesPanel() {
        ...
        content.getChildren().addAll(note, documentForm.root(), buildEntityForm(), buildFieldForm());
    ...
    private Parent buildEntityForm() {
        VBox box = section("Propiedades de entidad");
    ...
    private Parent buildFieldForm() {
        VBox box = section("Propiedades de campo");
    ...
    private Parent entityListGraphic(DataDictionaryEntity entity) {
        VBox box = new VBox(3);
        ...
        Label details = new Label(DataDictionaryLabels.label(entity.kind()) + " · "
    ...
    private void bindViewModel() {
        ...
        entityList.getSelectionModel().selectedItemProperty().addListener((observable, previous, current) -> {
            viewModel.selectedEntityProperty().set(current);
            refreshEntityForm(current);
            documentPreview.refresh();
        });
        viewModel.selectedEntityProperty().addListener((observable, previous, current) -> {
            if (entityList.getSelectionModel().getSelectedItem() != current) {
                entityList.getSelectionModel().select(current);
            }
            refreshEntityForm(current);
            documentPreview.refresh();
        });
    ...
    private void refreshEntityIndexSummary() {
        int entities = viewModel.entities().size();
        int fields = viewModel.currentDocument() == null ? 0 : viewModel.currentDocument().fieldCount();
        entityIndexSummary.setText(entities + " entidades · " + fields + " campos. El índice usa el alto natural del SideDock.");
    }
